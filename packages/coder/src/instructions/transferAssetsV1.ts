...
export type TransferAssetsV1InstructionData = {
...
export type TransferAssetsV1InstructionDataArgs = {
  roleId: number;
  authorityPayload: ReadonlyUint8Array;
};

export function getTransferAssetsV1InstructionDataCodec() {
  const encoder: Encoder<TransferAssetsV1InstructionDataArgs> =
    transformEncoder(
      getStructEncoder([
      ...
      (value) => ({
        ...value,
        discriminator: Discriminator.TransferAssetsV1,
        _padding: new Uint8Array(2),
      }),
    );

  const payloadEncoder: Encoder<
    Omit<TransferAssetsV1InstructionDataArgs, 'authorityPayload'>
  > = transformEncoder(
    getStructEncoder([
      ['discriminator', getSwigInstructionDiscriminatorEncoder()],
      ['_padding', fixEncoderSize(getBytesEncoder(), 2)],
  ...
  const decoder: Decoder<TransferAssetsV1InstructionData> = getStructDecoder([
