...
import { addAuthorityToSwig, createSwigWallet, deserializeSwigReference, findRootRole, refreshSwig, removeAuthorityFromSwig, serializeSwigReference, } from '@/services/swig';
import { useLocalStorage } from './useLocalStorage';
const SWIG_STORAGE_KEY = 'swig-wallet/active-swig';
export function useSwigManager(connection, payer) {
    const [storedReference, setStoredReference, clearStoredReference] = useLocalStorage(SWIG_STORAGE_KEY, () => null);
    const [swig, setSwig] = useState(null);
    ...
    const mutate = useCallback(async (executor) => {
        setMutationState('pending');
        setError(null);
        try {
    ...
    const addEd25519Authority = useCallback(async (params) => {
        if (!connection || !payer || !swig) {
            throw new Error('Wallet not ready');
        }
        return mutate(() => addAuthorityToSwig({
            connection,
            swig,
    ...
    const removeAuthority = useCallback(async (params) => {
        if (!connection || !payer || !swig) {
            throw new Error('Wallet not ready');
        }
        return mutate(() => removeAuthorityFromSwig({
            connection,
            swig,
            actingRoleId: params.actingRoleId,
    ...
    const replaceEd25519Authority = useCallback(async (params) => {
        if (!connection || !payer || !swig) {
            throw new Error('Wallet not ready');
        }
        return mutate(async () => {
    ...
    const activeReference = useMemo(() => {
        ...
        return { reference: storedReference, id, address };
    }, [storedReference]);
