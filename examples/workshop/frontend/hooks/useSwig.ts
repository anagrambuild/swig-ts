...
export function useSwig() {
  ...
  const createAccount = useCallback(async () => {
    ...
    try {
      ...
      await sendTransaction([createIx], userKeypair, [], connection);
      ...
      if (!rootRole) {
        throw new Error('Root role not found after creation');
      }
      ...
      await sendTransaction(addAuthorityIx, userKeypair, [], connection);
  ...
  const fundAccount = useCallback(
    async (amount: number) => {
      ...
      try {
        // STEP 2: Airdrop SOL to the Swig wallet address (only works on local validator)
        await connection.rpc
          .requestAirdrop(
            account.walletAddress,
            lamports(BigInt(amount * Number(LAMPORTS_PER_SOL))),
          )
          .send();
    ...
    async (backendAddress: Address) => {
      ...
      try {
        ...
        if (!managerRole.actions.canManageAuthority()) {
          throw new Error('Manager role cannot manage authority');
        }
        ...
        const addBackendAuthorityIx = await getAddAuthorityInstructions(
          ...
          backendActions,
          { payer: account.managerKeypair.address },
        );
  ...
  const refreshSwig = useCallback(async () => {
    ...
    try {
      await account.swig.refetch();
      const walletAddress = await getSwigWalletAddress(account.swig);
      setAccount({
        ...account,
        swig: account.swig,
        walletAddress,
      ...
      console.error('Error refreshing Swig:', err);
    }
  }, [account]);
